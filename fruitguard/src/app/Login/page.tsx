...
export default function Login(){
  const {submitLogin, loading, error, success} = useFetchLogin();
  ...
  useEffect(()=>{
    if(success){
      const timer = setTimeout(() => router.push('/Sidebar'), 0);
      ...
      <div className="w-1/2 flex items-center justify-center">
        <div className="w-full max-w-md p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {success && <p className="text-yellow-950 text-sm">Login successful! Redirecting...</p>}
            <div>
              <label className="block text-sm font-bold mb-1">Email</label>
              <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Enter email"
                className="w-full p-2 border rounded disabled:opacity-50" disabled={loading}/>
            </div>
            <div>
          ...
          <p className="mt-4 text-sm">
            ...
            <Link href="/Register" className="text-yellow-950 font-bold hover:underline">Sign up</Link>
          </p>
        </div>
      </div>
    </div>
